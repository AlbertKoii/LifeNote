import {
  ...
  interface NoteInputCardProps {
    ...
    date: [
      ...
      noteID: {
        noteST: boolean,
        noteInner: string[], // NoteInner should be an array of strings
      },
    ];
  
    onSubmit: (data: NoteInputCardProps) => void;
    onClose: () => void; // Define onClose as a function type
  }
  ...
  const NoteInputCard = ({ onSubmit, onClose }: NoteInputCardProps) => {
    ...
    const closeCardModel = () => {
      setCardOpen(false);
      onClose(); // Call the onClose function when closing the card model
    };
