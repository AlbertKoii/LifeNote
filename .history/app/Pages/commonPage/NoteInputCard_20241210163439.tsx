    ...
    const addNoteData = async () => {
      const newNote = {
        accountingId: {
            ...
            accountMemo: "note data",
            accountCheckSt: false,
            accountInner: "note data",
            accountAmount: 0
        }
        ,
        noteID: {
      ...
      try {
        const response = await axios.post("http://localhost:3000/accounts/1",newNote);
        console.log(response.data);
        const currentData = response.data.date
        currentData.date.push(newNote);

        await axios.put("http://localhost:3000/accounts/1", currentData);
