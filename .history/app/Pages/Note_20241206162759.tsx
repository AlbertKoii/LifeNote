...
const Note: React.FC<NoteProp> =()=>{
    ...
    return(
        <div className="mt-20 ml-10 mr-10">
            <div className="mb-10">
                <button className="border-gray-950 border-2 rounded-3xl p-3">新增記事</button>
                <button className="border-gray-950 border-2 rounded-3xl p-3">刪除記事</button>
            </div>
            {
    inner.length > 0 ? 
        inner.map((item, index) => (
            <div key={index} className="border p-2 flex items-center">
                {
                    item.noteID && (
                        <div className="grid grid-cols-2 gap-2 w-full">
                            <Checkbox 
                                className="m-0"
                                key={item.noteID.noteCheckSt}
                            />
                            <div className="ml-2" key={item.noteID.id}>
        ...
        )) : <div>Something error that can't load data</div>
}
